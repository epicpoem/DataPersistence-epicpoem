...
## [2026-06-12] 주문 등록 시 시료 존재 여부 검증 추가

### 작업 내용
- main.cpp doCreateOrder(): sampleRepo를 파라미터로 추가, 시료 ID 입력 직후 existsById() 검증 추가
- 존재하지 않는 시료 ID 입력 시 "[오류] 존재하지 않는 시료 ID입니다." 출력 후 즉시 반환
- 빌드 성공, 단위 테스트 27/27 통과

### 커밋
- `ceaf2b1` [AI-Fix] 주문 등록 시 시료 존재 여부 검증 추가

### 리뷰 요청
- 없는 시료 ID로 주문 등록 시도 시 에러 안내 동작 확인 부탁드립니다.

---
### 리뷰 (by User)
- 시료 ID 없을시 정상 에러처리 확인
...
### 작업 내용
- 리포지토리 최상위에 README.md 추가
- 포함 항목: 프로젝트 개요, 기술 스택, 프로젝트 구조, 계층 구조 다이어그램
- 포함 항목: 빌드 방법 (Visual Studio / MSBuild 명령줄)
- 포함 항목: 실행 방법, config.json 설정
- 포함 항목: 실행 결과 예시 (시료 등록/조회, 주문 등록/오류처리/상태수정, 재고수정, 영속성 확인)
- 포함 항목: 테스트 실행 방법 및 49건 테스트 구성 표
- 포함 항목: 주요 설계 결정 (JSON 저장, Repository 분리, Service 레이어, IClock 추상화)
- 포함 항목: 엔티티 정의 (Sample, Order 필드 설명)

### 커밋
- `1770031` [AI-Docs] Add README.md

### 리뷰 요청
- README 내용 및 구성 확인 부탁드립니다.
- DataPersistence PoC 전체 완료 여부 확인 요청
